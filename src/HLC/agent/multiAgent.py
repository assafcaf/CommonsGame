...
class MultiAgent:
    ...
    def __init__(self, input_shape, num_actions, num_agents, ep_steps, minibatch_size=64, update_frequency=25,
                 agent_history_length=4, capacity=int(1.5e5), lr=1e-4, replay_start_size=10000, model_name="",
    ...
    def get_actions_sm(self, states: dict, total_step):
        """Get action by ε-greedy method.
        ...
        Returns:
            action (tf.int32): Action index
        """
        actions = {agent_id: agent.get_action_sm(states[agent_id]) for agent_id, agent in self.agents.items()}
        return actions
